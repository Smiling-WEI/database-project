...
CREATE TABLE flight_instance (
    ...
    dep_time DATETIME NOT NULL COMMENT '计划起飞时间',

    arr_time DATETIME NOT NULL COMMENT '计划到达时间',

    aircraft_model VARCHAR(50) NOT NULL,

    first_seats INT UNSIGNED NOT NULL,

    economy_seats INT UNSIGNED NOT NULL,
    ...
    FOREIGN KEY(flight_no)
        REFERENCES flight_no_info(flight_no),
...
CREATE TABLE user (
    ...
    airline_id INT DEFAULT NULL COMMENT '仅航司管理员使用',
    ...
    FOREIGN KEY(airline_id)
        REFERENCES airline_company(airline_id),

    CONSTRAINT chk_airline_admin_has_airline
    CHECK (
        role <> '航司管理员'
        OR airline_id IS NOT NULL
    ),

    CONSTRAINT chk_system_admin_no_airline
    CHECK (
        role <> '系统总管理员'
        OR airline_id IS NULL
    ),

    CONSTRAINT chk_admin_role_only_for_airline_admin
    CHECK (
        role = '航司管理员'
...
CREATE TABLE active_ticket_sale (
    ...
    FOREIGN KEY(pricing_id)
        REFERENCES cabin_pricing(pricing_id)
...
CREATE TABLE refund_rule (
    ...
    max_hours_before_departure INT DEFAULT NULL,
...
CREATE TABLE refund_record (
    ...
    original_ticket_price DECIMAL(10,2)
        NOT NULL,
    ...
    FOREIGN KEY(order_id)
        REFERENCES archive_ticket_sale(order_id),

    FOREIGN KEY(rule_id)
        REFERENCES refund_rule(rule_id),

    FOREIGN KEY(operator_user_id)
        REFERENCES user(user_id),

    FOREIGN KEY(operator_admin_id)
        REFERENCES user(user_id)
